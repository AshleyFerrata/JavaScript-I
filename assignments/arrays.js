...
let carModels = [inventory.length];
for (let i = 0; i < inventory.length; i++) {
    carModels[i] = inventory[i].car_model;
}
carModels.sort();
console.log("Car models in alphabetical order:");
...
let BMWAndAudi =[inventory.length];
let index = 0;
for (let i = 0; i < inventory.length; i++) {
    if (inventory[i].car_make === "BMW" || inventory[i].car_make === "Audi") {
        BMWAndAudi[index] = inventory[i];
        index++;
    }
}
console.log("BMW and Audi cars:", JSON.stringify(BMWAndAudi, null, 2));
